...
public:
	...
	static void setWindow(const std::string &name)
	{
		...
		texture = textrenderer.generateFont(U"abcdefghijklmnopqrstuvwxyz ", 40);
	}
	...
	void exec_()
	{
		...
		while (!glfwWindowShouldClose(window))
		{
			...
			textrenderer.renderText(U"hello the world\nhello", TextShader,1.5,50);
			textrenderer.renderText(U"hello the world\nhello", TextShader,1.5,50,200);
...
int main()
{
	...
	glm::vec2 vh = app.textrenderer.getVHBBox(U"hello the world\nhello",1.5);
	Button btn5(50, 0, vh.x, vh.y, glm::vec3(0.0f, 0.0f, 0.0f), app.root.get());
	btn2.update(80, 10);
